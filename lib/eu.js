...
var debugHeaders = require('debug')('eu:headers');
  ...
  this.get = function (uri, options, callback) {
    ...
    cache.get(uri, function (err, value) {
      ...
      request(uri, options, function (err, res, body) {
        ...
        debugHeaders('%s headers: %s', uri, JSON.stringify(res.headers, null, 2));
        ...
        if (isCacheable) {
          var ttlMillis = Math.max((expiryTimeMillis || FOREVER_MILLIS) - Date.now(), 0);
          var cachedResponse = { response: cachedResponseProperties(res), expiryTimeMillis: expiryTimeMillis };

          debug('%s writing response to cache with ttl %s ms (%s)', uri, ttlMillis, new Date(ttlMillis + Date.now()));
